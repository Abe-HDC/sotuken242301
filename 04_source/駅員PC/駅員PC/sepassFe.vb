...
Public Class sepassFe
    ...
    Dim fare As Integer
    Private Sub sepassFe_Load(sender As Object, e As EventArgs) Handles Me.Load
        DisplayBalance()
        '変数の宣言
        ...
        Command.CommandText = $"SELECT s1.stationname,s2.stationname,fareusu.fare FROM iccard JOIN station AS s1 ON iccard.arpass = s1.stationname JOIN station AS s2 ON s2.stationname = '{ends}' JOIN com ON com.busid IN (SELECT busid FROM ruote WHERE ruoid = s1.ruoid) AND com.busid IN (SELECT busid FROM ruote WHERE ruoid = s2.ruoid) JOIN fareusu ON com.busid = fareusu.busid JOIN medium ON fareusu.medium = medium.mediumno WHERE fareusu.medium = 1 AND ABS(s2.pointkm - s1.pointkm) >= fareusu.minkm AND ABS(s2.pointkm - s1.pointkm) < fareusu.maxkm AND iccard.ICno = {ReceivedId}"
        'SQLを実行
